...
class ActorCriticNetwork(nn.Module):
    ...
    def forward(self, state):
        ...
        state_features = self.state_net(position)  # [1, 32]

        # Combine CNN and state features
        combined_features = torch.cat([cnn_features, state_features], dim=1)  # [1, cnn_out_size + 32]
        shared_features = self.shared_net(combined_features)  # [1, 64]
